...
public class ProductsController : BaseController
{
    ...
    [HttpGet("categories")]
    [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status200OK)]
    public async Task<IActionResult> CategoriesOfProduct(CancellationToken cancellationToken)
    {
        var listCategoriesResult = await _mediator.Send(new ListAllCategoriesOfProductsCommand(), cancellationToken);
        return Ok(listCategoriesResult, "Categories list retrieved successfully");
    }
